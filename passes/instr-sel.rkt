     ...
     (let-values ([(reg-args stack-args) (split-at-max args (length arg-regs))])
       ...
       (let ([stack-args (reverse stack-args)])
         ...
         `(; Save caller-save registers
           ;,@(map (lambda (save) `(pushq ,save)) arg-regs)
           ...
           ; Bump stack register if we pushed args on stack
           ,@(if (eq? (length stack-args) 0)
               `()
               `((addq (int ,(* 8 (length stack-args))) (reg rsp))))

           ; Load caller-save registers
           ;,@(map (lambda (save) `(popq ,save)) arg-regs)

           ; Move return value to it's destination
           (movq (reg rax) ,(arg->x86-arg bind-to)))))]
