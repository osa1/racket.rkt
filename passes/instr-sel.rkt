...
(define (instr-sel-expr new-vars bind-to expr)
  (match expr
    ...
    [`(app ,f . ,args)
     ...
     ; FIXME: We should pass arguments to rts functions in registers even when
     ; testing with registers disabled.
     (define use-regs?
       (match f
         [`(toplevel-fn print_int) #t]
         [_ (use-regs)]))

     (let-values ([(reg-args stack-args) (split-at-max args (length (if use-regs? arg-regs '())))])
